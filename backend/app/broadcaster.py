...
class ConnectionManager:
    ...
    async def start_pubsub(self):
        ...
        if not stream_writer.redis_client:
            return
        pubsub = stream_writer.redis_client.pubsub()
    ...
    async def broadcast(self, tenant_id: str, plant_id: str, message: dict):
        ...
        """
        if stream_writer.redis_client:
            channel = f"ws:broadcast:{tenant_id}:{plant_id}"
            await stream_writer.redis_client.publish(channel, json.dumps(message))
        else:
