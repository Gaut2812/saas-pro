...
async def setup_alarm_consumer_group(stream_key: str):
    ...
    try:
        await stream_writer.redis_client.xgroup_create(
            name=stream_key,
...
async def get_active_streams() -> List[str]:
    ...
    async for key in stream_writer.redis_client.scan_iter(match=pattern):
        streams.append(key)
    return streams
...
async def alarm_consumer_worker(worker_id: int):
    ...
    while True:
        try:
            ...
            messages = await stream_writer.redis_client.xreadgroup(
                groupname=ALARM_CONSUMER_GROUP,
            ...
            for stream_name, stream_messages in messages:
                ...
                try:
                    ...
                    await stream_writer.redis_client.xack(stream_name, ALARM_CONSUMER_GROUP, *msg_ids)
                except Exception as e:
